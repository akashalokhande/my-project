...
function Restaurant() {
  ...
  let getRestaurantDetails = async () => {
    let url = "http://localhost:5008/api/get-restaurant-details-by-id/" + id;
  ...
  let getMenuItems = async () => {
    let url = `http://localhost:5008/api/get-menu-items/${id}`;
    let { data } = await axios.get(url);
    console.log(data);
    if (data.status === true) {
  ...
  let makePayment = async () => {
    ...
    let url = "http://localhost:5008/api/gen-order-id";
    ...
    var options = {
      ...
      handler: async function (response) {
        ...
        let { data } = await axios.post(
          "http://localhost:5008/api/verify-payment",
          verifyData
        );
        if (data.status === true) {
